...
public class Main {
    ...
    /**
     * Single Responsibility (單一職責): A class should have one, and only one,
    ...
    /**
     * Open-Closed (開閉): A class should be open for extension, but closed for
     * modification.
    ...
    /**
     * Liskov Substitution (里氏替換): Subtypes should be replaceable by their base
     * types.
    ...
    /**
     * Interface Segregation (接口隔離): Many specific interfaces are better than a
     * general interface.
     * 客戶端不應該依賴於它們不需要的介面。
     * 如何應用：
     * - 建立小而具體的介面，而不是大而全的介面。
    ...
    /**
     * Dependency Inversion (依賴倒置): We must depend on abstractions and not concrete
     * classes.
